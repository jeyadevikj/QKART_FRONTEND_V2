...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      setAddresses({ ...addresses, all: data.data });
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      const deleteAddrs = await axios.delete(
      ...
      setAddresses({ all: deleteAddrs.data });
      console.log("delete", deleteAddrs);
    } catch (e) {
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {addresses.all.length ? (
                addresses.all.map((res, id) => {
                  return (
                    <Box
                      className={`address-item ${
                        addresses.selected === res._id
                          ? "selected"
                          : "not-selected"
                      }`}
                      onClick={() =>
                        setAddresses({ ...addresses, selected: res._id })
                      }
                    >
                      <Typography>{res.address}</Typography>
                      <Button
                        variant="text"
                        startIcon={<Delete />}
                        onClick={() => {
                          deleteAddress(token, res._id);
                        }}
                      >
                        Delete
                      </Button>
